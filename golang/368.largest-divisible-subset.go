...
func largestDivisibleSubset(nums []int) []int {
	...
	for i := range nums {
		...
		for j := i - 1; j >= 0; j-- {
			if nums[i]%nums[j] == 0 {
				if len(dp[j][0]) > tmpMaxLen {
					tmpMaxLen = len(dp[j][0])
					tmpMaxInd = []int{j}
				} else if len(dp[j][0]) == tmpMaxLen {
					tmpMaxInd = append(tmpMaxInd, j)
				}
				find = true
			}
		}
		if !find {
			...
			for j := range tmpMaxInd {
				for k := range dp[tmpMaxInd[j]] {
					dp[i] = append(dp[i], append([]int{}, append(dp[tmpMaxInd[j]][k], nums[i])...))
				}
			}
		}
		if len(dp[i][0]) > maxLen {
			maxLen = len(dp[i][0])
			res = dp[i][0]
		}
	}
